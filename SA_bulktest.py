...
evaluation_record = SAFA.start_new_SA_record("BulkEval_Test")
print("The evaluation record has been created.")
response_list = SAFA.csv_to_list_of_dicts("Dataset/BulkTest_SAFA_sample.csv")
print("The response list has been created.")

for scenario_dict in response_list:
    new_row = list()
    subject, level, question, students_response, suggested_answer, max_marks, exp_marks = SAFA.SA_extract_parameters(scenario_dict)
    ...
    user_prompt = SAFA.assemble_SA_prompt(subject, level, question, suggested_answer, max_marks, students_response)
